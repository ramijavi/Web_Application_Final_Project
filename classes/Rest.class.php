	...
	//				print "\n Read Membership...\n";
			...
			return $membership;
                ...
                public function getAllUsers($access_token) {
                        ...
                        $request = new HTTP_Request2($constants->HOSTNAME . $constants->USER_PATH, HTTP_Request2::METHOD_GET);
		...
		public function getAllCoursesForUser($access_token, $user_id) {
                        $constants = new Constants($this->clientURL);
                        //$GradebookGrades = new GradebookGrades();

                        $request = new HTTP_Request2($constants->HOSTNAME . '/users/' . $user_id . '/courses', HTTP_Request2::METHOD_GET);
                        ...
                        try {
                                ...
                                if (200 == $response->getStatus()) {
                                        ...
                                        $Classes = json_decode($response->getBody());
                                } else {
                                        print 'Unexpected HTTP status: ' . $response->getStatus() . ' ' .
                                                        $response->getReasonPhrase();
                                                        $BbRestException = json_decode($response->getBody());
		...
		public function getTotalGrade($access_token, $course_id, $column_id, $user_id) {
                        $constants = new Constants($this->clientURL);
                        //$GradebookGrades = new GradebookGrades();

                        $request = new HTTP_Request2($constants->HOSTNAME . '/courses/' . $course_id . '/gradebook/columns/' . $column_id . '/users/' . $user_id, HTTP_Request2::METHOD_GET);
